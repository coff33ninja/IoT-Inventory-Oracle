...
const LinkInventoryModal: React.FC<LinkInventoryModalProps> = ({
  isOpen,
  onClose,
  entity,
}) => {
  ...
  const availableInventory = useMemo(() => {
    return inventory
      .filter((item) => item.status === "I Have")
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (entity && selectedItemId) {
      ...
      addToast(
        `Linked ${entity.attributes.friendly_name} to ${itemName}`,
        "success"
      );
      onClose();
    }
          ...
          <div className="flex justify-between items-center pb-4 border-b border-border-color">
            <div>
              ...
              <p className="text-sm text-text-secondary">
                to {entity.attributes.friendly_name || entity.entity_id}
              </p>
            </div>
            <button
              onClick={onClose}
