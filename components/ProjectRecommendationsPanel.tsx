...
const ProjectRecommendationsPanel: React.FC<
...
> = ({ project, onUpdateProject }) => {
  ...
  const [activeTab, setActiveTab] = useState<
    "alternatives" | "optimizations" | "missing"
  >("alternatives");
  ...
  const loadProjectRecommendations = async () => {
    ...
    try {
      ...
      for (const component of project.components) {
        ...
        if (component.inventoryItemId) {
          ...
          if (inventoryItem) {
            ...
            for (const alternative of alternatives.slice(0, 2)) {
              ...
              const altItem = inventory.find(
                (item) => item.id === alternative.componentId
              );
              if (altItem && altItem.id !== inventoryItem.id) {
                allRecommendations.push({
                  ...
                  priority:
                    alternative.compatibilityScore > 85
                      ? "high"
                      : alternative.compatibilityScore > 70
                      ? "medium"
                      : "low",
                  estimatedSavings:
                    alternative.priceComparison.savings > 0
                      ? alternative.priceComparison.savings
                      : undefined,
                });
              }
            }
        ...
        const optimizations = await analyzeComponentOptimizations(
          component,
          inventory
        );
      ...
      const missingComponents = await suggestMissingComponents(
        project,
        inventory
      );
        ...
        if (availableQty > component.quantity * 3 && component.quantity === 1) {
          optimizations.push({
            ...
            suggestion: {
              ...
              confidence: 0.7,
              reasoning: `Consider using ${Math.min(
                availableQty,
                component.quantity + 2
              )} units for redundancy and future modifications`,
              priority: "medium",
  ...
  const suggestMissingComponents = async (
    ...
    const componentNames = currentProject.components.map((c) =>
      c.name.toLowerCase()
    );

    // If project has microcontroller but no basic components
    const hasMicrocontroller = componentNames.some(
      (name) =>
    ...
    if (hasMicrocontroller) {
      ...
      const hasResistor = componentNames.some((name) =>
        name.includes("resistor")
      );
      const hasBreadboard = componentNames.some((name) =>
        name.includes("breadboard")
      );

      if (!hasLED) {
        const ledItems = availableItems.filter(
          (item) =>
        ...
        if (ledItems.length > 0) {
          ...
          missing.push({
            ...
            suggestion: {
              ...
              confidence: 0.8,
              reasoning:
                "LEDs are essential for visual feedback and debugging in microcontroller projects",
              priority: "high",
              estimatedCost: ledItem.purchasePrice || 0.5,
              alternatives: [],
            },
            reasoning:
              "Most microcontroller projects benefit from LED indicators for status and debugging.",
            priority: "high",
          });
        }
      }
      ...
      if (!hasResistor) {
        const resistorItems = availableItems.filter(
          (item) =>
        ...
        if (resistorItems.length > 0) {
          ...
          missing.push({
            ...
            suggestion: {
              ...
              confidence: 0.9,
              reasoning:
                "Resistors are needed for current limiting and pull-up/pull-down configurations",
              priority: "high",
              estimatedCost: (resistorItem.purchasePrice || 0.1) * 2,
              alternatives: [],
            },
            reasoning:
              "Resistors are essential for protecting components and proper circuit operation.",
            priority: "high",
          });
        }
      }
      ...
      if (!hasBreadboard) {
        const breadboardItems = availableItems.filter(
          (item) =>
        ...
        if (breadboardItems.length > 0) {
          ...
          missing.push({
            ...
            suggestion: {
              ...
              confidence: 0.85,
              reasoning:
                "Breadboards enable rapid prototyping and circuit testing without soldering",
              priority: "medium",
              estimatedCost: breadboardItem.purchasePrice || 5,
              alternatives: [],
            },
            reasoning:
              "A breadboard will make prototyping and testing much easier.",
            priority: "medium",
          });
        }
      }
    ...
    try {
      if (recommendation.type === "alternative") {
        ...
        const newComponent = {
          ...
          name: recommendation.suggestion.name,
          quantity:
            "quantity" in recommendation.suggestion
              ? recommendation.suggestion.quantity
              : 1,
          source: "inventory" as const,
          inventoryItemId: recommendation.suggestion.componentId,
          isAllocated: true,
        ...
        {[
          {
            id: "alternatives",
            label: "Alternatives",
            count: getRecommendationsByType("alternatives").length,
          },
          {
            id: "optimizations",
            label: "Optimizations",
            count: getRecommendationsByType("optimizations").length,
          },
          {
            id: "missing",
            label: "Missing Components",
            count: getRecommendationsByType("missing").length,
          },
        ].map((tab) => (
          <button
            key={tab.id}
        ...
        {loading ? (
          ...
          <>
            {getRecommendationsByType(activeTab).length > 0 ? (
              getRecommendationsByType(activeTab).map(
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <h4 className="font-medium text-text-primary flex items-center">
                            ...
                            <span
                              className={`ml-2 px-2 py-1 rounded text-xs border ${getPriorityColor(
                    ...
                    <div className="flex items-center justify-between text-xs text-text-secondary">
                      <div className="flex space-x-4">
                        ...
                        <span>
                          Confidence:{" "}
                          {Math.round(
                            recommendation.suggestion.confidence * 100
                          )}
                          %
                        </span>
                        {"estimatedCost" in recommendation.suggestion &&
                          recommendation.suggestion.estimatedCost > 0 && (
                            <span>
                              Cost:{" "}
                              {formatCurrency(
                                recommendation.suggestion.estimatedCost
                              )}
                            </span>
                          )}
                      </div>
              ...
              <div className="text-center py-8 text-text-secondary">
                <p className="text-sm">
                  {activeTab === "alternatives" &&
                    "No alternative components found."}
