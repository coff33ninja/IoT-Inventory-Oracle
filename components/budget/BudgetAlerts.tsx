...
const BudgetAlerts: React.FC<BudgetAlertsProps> = ({
  ...
  const generateAlerts = (): Alert[] => {
    ...
    if (spendingAnalysis) {
      if (spendingAnalysis.budgetEfficiency < 25) {
        alerts.push({
          ...
          message: `Budget efficiency is low at ${spendingAnalysis.budgetEfficiency.toFixed(
            1
        ...
        alerts.push({
          ...
          message: `Budget efficiency is moderate at ${spendingAnalysis.budgetEfficiency.toFixed(
            1
    ...
    if (expensiveProjects.length > 0) {
      alerts.push({
        ...
        message: `${expensiveProjects.length} project${
          expensiveProjects.length !== 1 ? "s" : ""
        } with costs over ${formatCurrency(500)} detected.`,
    ...
    const expensiveItems = inventory.filter(
      (item) => item.purchasePrice && item.purchasePrice > 100
    );
    ...
    if (expensiveItems.length > 5) {
      alerts.push({
        ...
        message: `${expensiveItems.length} items worth over ${formatCurrency(
          100
        )} each in inventory. Consider usage optimization.`,
    ...
    const unusedItems = inventory.filter(
      (item) => !item.usedInProjects || item.usedInProjects.length === 0
    );

    if (unusedItems.length > 10) {
      const unusedValue = unusedItems.reduce(
      ...
      alerts.push({
        ...
        message: `${unusedItems.length} items (worth ${formatCurrency(
          unusedValue
        )}) haven't been used in any projects.`,
    ...
    if (spendingAnalysis && spendingAnalysis.budgetEfficiency > 75) {
      alerts.push({
        ...
        message: `Great job! Your budget efficiency is ${spendingAnalysis.budgetEfficiency.toFixed(
          1
        )}%.`,
        severity: "low",
      });
  ...
  const getAlertStyles = (type: Alert["type"]) => {
    switch (type) {
      ...
      default:
        return "bg-secondary border-border-color text-text-primary";
    }
  };
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-text-secondary">
              No budget alerts at this time. Keep up the good work!
            </p>
          </div>
        </div>
      </div>
