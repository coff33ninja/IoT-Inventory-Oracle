...
const NavItem: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`flex items-center w-full ${isCollapsed ? 'px-3 py-3 justify-center' : 'px-4 py-3'} text-left rounded-lg transition-colors duration-200 ${
      isActive
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
        ...
        ></div>
        <aside
            className={`fixed top-0 left-0 z-40 ${isCollapsed ? 'w-16' : 'w-64'} h-full bg-secondary border-r border-border-color flex flex-col p-4 transition-all duration-300 ease-in-out ${
            isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
            } lg:translate-x-0`}
        >
            <div className={`flex items-center mb-8 px-2 ${isCollapsed ? 'justify-center flex-col' : ''}`}>
                {isCollapsed ? (
