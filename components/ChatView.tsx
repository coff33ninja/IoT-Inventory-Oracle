...
const ChatView: React.FC<ChatViewProps> = ({ initialMessage }) => {
  ...
  const handleAddToInventory = async (part: AiSuggestedPart, status: ItemStatus) => {
      ...
      if (inventoryItem) {
        ...
        if (allocateQty > 0) {
          ...
          if (aiComp.quantity > allocateQty) {
            projectComponents.push({
              id: `ai-${Date.now()}-${aiComp.name.replace(/\s/g, "")}-remainder`,
              name: aiComp.name,
              quantity: aiComp.quantity - allocateQty,
              source: "ai-suggested" as const,
            });
            missingComponents.push(`${aiComp.quantity - allocateQty} x ${aiComp.name}`);
          }
        } else {
